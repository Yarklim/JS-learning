// [1, 2, 3].push(4) // [1, 2, 3, 4] - добавляет в конец массива
// [1, 2, 3].pop() // [1, 2] - удаляет из конца массива
// [1, 2, 3].shift() // [2, 3] - удаляет первый элемент массива
// [1, 2, 3].unshift(5) // [5, 1, 2, 3] - добавляет элемент в начало массива
// ['a', 'b'].concat('c') // ['a', 'b', 'c'] - объединяет массивы в один массив
// const name = "Mango";
// console.log(name.split("")); // ["M", "a", "n", "g", "o"] - создает массив из строки через разделитель
// ['a', 'b'].join('-') // 'a-b' - создает строку из массива
// ['a', 'b', 'c'].slice(1) // ['b', 'c'] - возвращает новый массив, начиная с индекса, указзанного в первом параметре, и до (не включая) индекса второго параметра
// [3, 5, 6, 8].splice(1, 1) // [3, 6, 8] - удаление
// [3, 5, 6, 8].splice(1, 1, 4) // [3, 4, 6, 8] - замена
// [3, 5, 6, 8].splice(1, 0, 4) // [3, 4, 5, 6, 8] - добавление
// ['a', 'b', 'c'].indexOf('b') // 'b' - возвращает индекс найденого элемента или -1, если элемент не найден
// ['a', 'b', 'c'].includes('c') // true - находит параметр в массиве, возвращает true или false
...
В массиве students хранится список студентов со списком предметов которые посещает студент в свойстве courses.Несколько студентов могут посещать один и тот же предмет. 
...
то метод find(callback) позволяет найти и вернуть первый подходящий элемент, после чего перебор массива прекращается.То есть он ищет до первого совпадения.
...
//? -----------Массив объектов------------
/*
При работе с массивом объектов проверяется значение какого - то их свойства Например, есть массив объектов фруктов,
