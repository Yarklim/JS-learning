...
// Никогда не использовать стрелки как метод объекта, потому что они навсегда запоминают внешний контекст!
// СТрелки не могут быть функциями конструкторами! У стрелок нет внутреннего метода Construct.
...
// ? Возврат объекта в стрелочной функции:
const arrowObj = () => ({ a: 5 }); // обернуть в скобки, иначе интрепретатор будет воспринимать {} как тело функции
...
// Контекст стрелки невозможно поменять, она навсегда запомнила контекст в момент своего объявления.

const showThis = () => {
  console.log('this in showThis: ', this); // undefined
};
...
user.showContext(); // до сих пор будет this in showThis: window
