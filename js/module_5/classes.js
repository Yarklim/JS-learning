...
// Но часто нужно создать много однотипных объектов с одинаковым набором свойств, но разными значениями и методами для взаимодействия с ними.
// Всё это нужно сделать динамичекски, во время выполнения программы.Для этого используют классы - специальный синтаксис объявления функции для создания объектов.
...
const mango4 = new User('Манго', 'mango@mail.com');
console.log(mango4); // { name: 'Манго', email: 'mango@mail.com' }

const poly2 = new User('Поли', 'poly@mail.com');
...
mango2.changeEmail('mango@supermail.com');
console.log(mango2.getEmail()); // mango@supermail.com
console.log(mango2.#email); // Будет ошибка, это приватное свойство
...
const mango1 = new User({ name: 'Манго', email: 'mango@mail.com' });
console.log(mango1.email); // mango@mail.com
mango1.email = 'mango@supermail.com';
console.log(mango1.email); // mango@supermail.com

// При обращении к mango.email вызызвается геттер get email() {... } и выполняется его код.
// При попытке записи mango.email = "mango@supermail.com" вызывается сеттер set email(newEmail) {... } и строка "mango@supermail.com" будет значением параметра newEmail.
...
const editor2 = new ContentEditor({ email: 'mango@mail.com', posts: [] });
